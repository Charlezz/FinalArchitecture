...
public class HtmlTagHandler implements WrapperTagHandler {
    ...
    public static final String IFRAME_ITEM = "HTML_TEXTVIEW_ESCAPED_IFRAME";
    ...
    String overrideTags(@Nullable String html) {
        ...
        html = html.replace("</a>", "</" + A_ITEM + ">");
        html = html.replace("<iframe","<"+IFRAME_ITEM);
        html = html.replace("</iframe>","</"+IFRAME_ITEM+">");

        html = html.replaceAll("\n","<br />");
        html = html.replaceAll("<pre class=\"[\\s\\S]+?\">","<code>");
        html = html.replaceAll("</pre>", "</code>");
        Pattern pattern = Pattern.compile("<code>((?:.|\\s)+?)</code>");
        Matcher matcher = pattern.matcher(html);
        while(matcher.find()){
            String group = matcher.group();
            html = html.replace(group, group.replaceAll("    ", "&nbsp;&nbsp;"));
        }
        return html;
    }
    ...
    public boolean handleTag(boolean opening, String tag, Editable output, Attributes attributes) {
        if (opening) {
            ...
            if (HtmlTextView.DEBUG) {
                Log.d(HtmlTextView.TAG, "opening, tag="+tag+" output: " + output.toString());
            }
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                ...
                start(output, new Td());
            } else if(tag.equalsIgnoreCase(IFRAME_ITEM)){
                final String src = attributes != null ? attributes.getValue("src") : null;
                start(output, new IFrame(src));
            }
            else {
                return false;
            }
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                    ...
                    public void onClick(View widget) {
                        if (onClickATagListenerProvider != null) {
                            ...
                            if(onClickATagListener!=null){
                                clickConsumed = onClickATagListener.onClick(widget, spannedText, getURL());
                            }
                            if (!clickConsumed) {
                ...
                end(output, Code.class, false, new TypefaceSpan("monospace"), new RelativeSizeSpan(0.7f));
            } else if (tag.equalsIgnoreCase("center")) {
                ...
                final Object iframe = getLast(output, IFrame.class);
                final int spanStart = output.getSpanStart(iframe);
                final int spanEnd = output.length();
                final String src = iframe instanceof IFrame ? ((IFrame) iframe).src : null;
                final String name = iframe instanceof IFrame ? ((IFrame) iframe).type.name :null;
                final String spannedText = output.subSequence(spanStart, spanEnd).toString();
                end(output.append(name), IFrame.class, false, new URLSpan(src) {
                    @Override
                    public void onClick(View widget) {
                        if (onClickATagListenerProvider != null) {
                            OnClickATagListener onClickATagListener = onClickATagListenerProvider.provideTagClickListener();
                            boolean clickConsumed = false;
                            if(onClickATagListener!=null){
                                clickConsumed = onClickATagListener.onClick(widget, spannedText+"Youtube", getURL());
                            }
                            if (!clickConsumed) {
                                super.onClick(widget);
                            }
