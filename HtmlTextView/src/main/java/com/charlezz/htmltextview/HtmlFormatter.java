...
public class HtmlFormatter {
    ...
    public static Spanned formatHtml(@Nullable String html, ImageGetter imageGetter, ClickableTableSpan clickableTableSpan, DrawTableLinkSpan drawTableLinkSpan, TagClickListenerProvider tagClickListenerProvider, float indent, boolean removeTrailingWhiteSpace) {
        ...
        if (removeTrailingWhiteSpace) {
            formattedHtml = removeHtmlBottomPadding(HtmlCompat.fromHtml(html, HtmlCompat.FROM_HTML_MODE_LEGACY,imageGetter, new WrapperContentHandler(htmlTagHandler)));
        } else {
            formattedHtml = HtmlCompat.fromHtml(html, HtmlCompat.FROM_HTML_MODE_LEGACY,imageGetter, new WrapperContentHandler(htmlTagHandler));
        }
